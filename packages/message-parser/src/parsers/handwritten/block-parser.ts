...
import { parseInlineSegment } from './inline-parser';
import {
	blockquoteLine,
	codeFenceLanguage,
	headingMatch,
...
export const parseBlocks = (input: string, options?: Options): Root => {
	const tokens = tokenizeLines(input);
	const output: Root = [];
	const baseState = new ParserState(options);
	...
	for (let index = 0; index < tokens.length; ) {
		...
		if ((isBlockquoteMarker(current.value) && !isBlankBlockquoteMarker(current.value)) || canStartBlankBlockquote) {
			...
			while (cursor < tokens.length) {
				...
				paragraphs.push(paragraph(parseInlineSegment(blockLine, baseState)));
				cursor += 1;
		...
		if (taskLine(current.value) !== undefined) {
			const items: ReturnType<typeof task>[] = [];
			let cursor = index;
			...
			while (cursor < tokens.length) {
				...
				items.push(task(parseInlineSegment(item.text, baseState), item.status));
				cursor += 1;
		...
		if (orderedListLine(current.value) !== undefined) {
			...
			while (cursor < tokens.length) {
				...
				items.push(listItem(parseInlineSegment(item.text, baseState), item.number));
				cursor += 1;
		...
		if (unorderedListLine(current.value) !== undefined) {
			...
			while (cursor < tokens.length) {
				...
				items.push(listItem(parseInlineSegment(item.text, baseState)));
				cursor += 1;
		...
		output.push(paragraph(parseInlineSegment(current.value, baseState)));
		index += 1;
